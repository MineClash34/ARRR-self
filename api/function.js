...
module.exports = {
    ...
    "getMessage": async (channelId, messageId) => require("./apiCall.js")(`/channels/${channelId}/messages?limit=100`).then((msgs) => { var msg = msgs.find(m => m.id === messageId); if (msg === undefined) {console.log("SelfBot ERROR : Une erreur s'est produite ! Cela peut être du à un mauvais paramètre (ID de channel / ID de message) ou une erreur chez Discord. Si l'erreur persiste vérifiez bien votre code !"); return process.exit(1)} else {return msg}}),
        // React on a message
